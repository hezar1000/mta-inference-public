...
if __name__ == "__main__":
   ...
   # for var in ['effort_draws', 'true_grades', 'efforts']:
    for var in ['true_grades', 'efforts']:
        ...
        if var == 'true_grades':
            true_grade_histograms = convert_samples_to_bin_histograms(samples, np.array([0, 1, 2, 3, 4, 5]))
            true_grades_likely = true_grade_histograms.argmax(axis=0)
            summary[var]['bin'] = np.array([0, 1, 2, 3, 4, 5])[true_grades_likely]
            summary[var]['posterior_mass'] = np.transpose(true_grade_histograms[:,i,:])
