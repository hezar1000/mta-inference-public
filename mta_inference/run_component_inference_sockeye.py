...
import component_model_inference
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run Gibbs sampling for censored model.')
    # paths
    ...
    # parser.add_argument('--clamped_path', type=str, required=True, help='Location to read clamped values from') TODO: zombies with database
    ...
    #parser.add_argument('--alpha_tau', type=float, required=True)
    parser.add_argument('--alpha_tau', nargs='*', default= [], required=True)
    #parser.add_argument('--beta_tau',  type=float, required=True)
    parser.add_argument('--beta_tau', nargs='*', default=[], required=True)
    parser.add_argument('--alpha_e',   type=float, required=True)
    parser.add_argument('--beta_e',    type=float, required=True)
    parser.add_argument('--tau_l',     type=float, required=True)
    ...
    true_grades_gibbs, efforts_gibbs, reliabilities_gibbs, effort_draws_gibbs = component_model_inference.run_gibbs(
        reported_grades_instructor,
        graph_instructor, 
        hyperparams, 
        max_error=0, # unused parameter... 
    ...
    sample_data = {
        ...
        'efforts': efforts_gibbs,
        'reliabilities': reliabilities_gibbs,
        'effort_draws': effort_draws_gibbs,
