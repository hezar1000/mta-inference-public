...
def summarizeSamples(input_dir, sample_dir, output_dir, lb_quantile, num_samples_discard, true_grade_sample_bins, true_grade_output_bins):
    ...
    with bz2.BZ2File(input_dir,'rb') as f:
    ...
    for fname in glob.glob(os.path.join(sample_dir, '*.pbz2')):
        with bz2.BZ2File(fname, 'rb') as f:
            ...
            effort_draw_samples.append(samples['effort_draws'][num_samples_discard:].astype(int))
...
##################################
   ...
   # for fname in input_fnames:
    ...
    # window_size=config['inference_settings']['window_size'] - int(args.divide_by)
    ...
    sub_dir = 'no_zombies_dataset_num_'+str(args.dataset_num)
    ...
    with bz2.BZ2File(final_input_dir+'/redis.pbz2','wb') as f:
        # cPickle.dump(load_mta_data_from_redis(input_db, excluded_submissions=[], verbose=True),f)
        cPickle.dump([reported_grades, graph, ordered_graph, calibration_grades, effort_grades, week_nums, students, submissions],f, protocol=4)

    # with bz2.BZ2File(final_input_dir+'/zumbie_redis.pbz2','wb') as f:
    ...
    # export_effort_and_reliabilities(final_input_dir+'/efforts_and_reliabilities', dataset.efforts, dataset.reliabilities, zumbie_students , include_header=True)
